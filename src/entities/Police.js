...
export class Police {
  constructor(scene, x, y) {
    ...
    // Create sprite
    this.sprite = scene.physics.add.sprite(x, y, 'police_down');
  ...
  checkForPlayer(player) {
    ...
    // Detection based on distance and heat
    if (dist < this.sightRange) {
      // Higher heat = easier to spot
      const detectionChance = (heat / 100) * (1 - dist / this.sightRange);

      if (heat > 50 || (heat > 20 && Math.random() < detectionChance * 0.02)) {
  ...
  startChase(player) {
    ...
    this.stateTimer = 0;
    this.scene.events.emit('policeAlert', this);
  }
  ...
  catchPlayer(player) {
    ...
    this.isChasing = false;
    this.stateTimer = 0;
    this.scene.events.emit('playerCaught');
  }
  ...
  pickNewPatrolTarget() {
    const roadTiles = this.scene.cityGenerator.getRoadTiles();
  ...
  updateSprite() {
    const texBase = this.isChasing ? 'police_alert' : 'police';
