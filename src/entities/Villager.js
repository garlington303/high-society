...
export class Villager {
  constructor(scene, x, y) {
    ...
    // Create sprite
    this.sprite = scene.physics.add.sprite(x, y, `villager_${this.variant}_down`);
    this.sprite.setDepth(5);
    this.sprite.setData('entity', this);
    this.sprite.body.setCollideWorldBounds(true);

    // Scale villagers to match player/other entities
    ...
    // Snap to path and get initial waypoint
    this.snapToPath();
    this.updateWaypoint();
  }
  ...
  snapToPath() {
    ...
    // If current tile is not walkable, find nearest path
  ...
  update(delta) {
    ...
    this.sprite.setTexture(`villager_${this.variant}_${this.direction}`);
  }
  ...
  updateWaypoint() {
    ...
    // Try to get a sensible next path waypoint
    ...
    if (!waypoint || (waypoint.x === this.sprite.x && waypoint.y === this.sprite.y)) {
      // If no valid next tile, pick a random nearby path target
      this.pickRandomTarget();
      return;
    }
  ...
  pickRandomTarget() {
    const paths = this.scene.townGenerator.getPathTiles();
    if (paths.length > 0) {
      const target = Phaser.Math.RND.pick(paths);
  ...
  witness(crimeType) {
    ...
    if (Math.random() < (reportChance[crimeType] || 0.05)) {
      if (this.scene.infamySystem) this.scene.infamySystem.add(3, 'witnessed');
    }
  }
}
